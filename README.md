...
**Experimental pre-release: 0.3.0-rc.7**
...
| **Pallet Town** | Preserves Gen 1 visuals by default. FireRed ground is declared by target coordinate, not a repeated target block ID. Red’s House, Blue’s House, and Oak’s Lab each use only their verified two-by-two Gen 1 exterior footprint. It carries a private ADVANCED-colour variant so copied Gen 1 pixels are pre-baked with the same base OVERWORLD palette groups when Gen1Recomp reloads the map after applying the player’s saved colour option. | Existing coordinates, entrance behavior, collision, warps, and map transitions remain Gen 1-owned. |
...
> **v0.3.0-rc.7 rule:** the Gen 1 target map defines every coordinate and gameplay meaning. The constrained Pallet profile starts with the original Gen 1 pixel block and paints FireRed only at explicitly declared target coordinates; the original Gen 1 semantic tile stays locked for every block’s four movement cells.
...
Install **v0.3.0-rc.7** from its GitHub pre-release ZIP, enable it, and select the verified FireRed ROM when the launcher asks. The personal index remains on stable **v0.2.4** until this experiment is promoted. The package ID remains `FIRERED_KANTO_VISUALS`, so users of older uppercase releases can update normally.

**FR MAP VISUALS** is enabled by default. After installing v0.3.0-rc.7, fully restart the game before testing. This is a pre-release; return to stable v0.2.4 if any map transition, doorway, path, grass, or Route 1 connection becomes less reliable. If map visuals are enabled but **no** requested profile can build, the importer now fails visibly in the Mod Manager with the first precise reader/converter diagnostic instead of silently showing all-native terrain. The first test route should be as follows:
